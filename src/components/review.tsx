...
function Display({ rating, name, title, comment }: DisplayProps) {
    return (
        <blockquote>
            ...
            <footer className="mt-4">
                <p className="text-xs text-gray-500">by {name}</p>
            </footer>
        </blockquote>
    )
}
export default function Review() {
    ...
    const [reviews, setReviews] = useState<DisplayProps[]>([])

    useEffect(() => {
        // get the comments form local storage
        const comments = localStorage.getItem('comments')
        if (comments) {
            setReviews(JSON.parse(comments))
        }
    }, [])
